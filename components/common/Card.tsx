...
const Card: FC<Props> = ({ data, checked, active }) => {
  const activeClasses = active
    ? "ring-2 ring-brand-500 dark:ring-brand-600 dark:ring-offset-gray-700 ring-offset-1"
    : "";

  return (
    <div
      className={`flex flex-col justify-between h-full gap-6 p-4 md:p-6 border rounded-lg cursor-pointer ${
        checked ? "border-brand-600" : "border-gray-300 dark:border-gray-500"
      } ${activeClasses}`}>
      <div
        className={`w-8 h-8 ${
          checked ? "text-brand-600" : "text-gray-700 dark:text-gray-500"
        }`}>
        {data.icon}
      </div>
